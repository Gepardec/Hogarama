...
To get started locally, it is best to use only simple components for the runtime. For example, don't use Keycloak or Kafka. [See Modularisation Architecture](Modularisation-Architecture) and [Optional Modules](Optional-Modules). Compile e.g. with:
```
...
Download the [Wildfly Server](https://github.com/wildfly/wildfly/releases) (29.0.1.Final) and place the zip file in the `Downloads` folder of your user. In this case there is a `wildfly-29.0.1.Final.zip` in the Downloads folder.
...
Have a look to the [hogarama_local.env](https://github.com/Gepardec/Hogarama/blob/master/codereadyworkspace/hogarama_local.env) file, this is the configuration of your local hogajama.
...
To start all services go to [Docker-Infrastructure](https://github.com/Gepardec/Hogarama/tree/master/Docker-Infrastructure/) and run `docker-compose -p local-hogarama up -d`.
...
3. the `hogarama` credentials secret has to be created:
    ...
    4. Copy the new generated secret and set it in the `Docker-Infrastructure/hogarama/local_env/hogarama_local.env` file as `KEYCLOAK_CREDENTIALS_SECRET` environment variable
...
##### Local ENV file
Have a look at the `Docker-Infrastructure/hogajama/local_env/hogarama_local.env` file, this is the configuration of your local hogajama.
...
2. IntelliJ: Disable the `Pass environment variables` within the Run/Debug Configuration->Startup/Connection for your Wildfly Server
